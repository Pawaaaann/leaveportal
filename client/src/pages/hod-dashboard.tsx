...
export default function HODDashboard() {
  ...
  // Fetch approved requests filtered by HOD's department
  const { data: approvedApplications = [], isLoading: loadingApproved } = useQuery<LeaveRequest[]>({
    queryKey: ["/api/leave-requests/approved", userData?.dept],
    queryFn: async () => {
  ...
  // Fetch rejected requests filtered by HOD's department
  const { data: rejectedApplications = [], isLoading: loadingRejected } = useQuery<LeaveRequest[]>({
    queryKey: ["/api/leave-requests/rejected", userData?.dept],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (userData?.dept) params.append("department", userData.dept);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="ml-64 p-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">Review and approve department leave applications</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center">
                <h3 className="text-2xl font-bold text-foreground" data-testid="text-department-students">
        ...
        <Card className="mt-8">
          <CardHeader>
            <CardTitle>Approved Applications ({userData?.dept})</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="mt-8">
          <CardHeader>
            <CardTitle>Rejected Applications ({userData?.dept})</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full">
